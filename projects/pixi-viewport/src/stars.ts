...
function overlap(x: number, y: number, viewport: Viewport, starSize: number): boolean {
    const size = starSize;

    for (const child of viewport.children) {
        if (x < child.x + size && x + size > child.x && y < child.y + size && y + size > child.y) {
            return true;
        }
    }
...
export function stars(viewport: Viewport, starSize: number, border: number): void {
