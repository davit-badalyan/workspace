...
export function create(renderer: Renderer): void {
    // create the viewport
    ...
    setTimeout(() => {
        ...
        // viewport.animate({ scaleX: 0.5, scaleY: 0.5 });
    }, 5000);
}

function border(viewport: Viewport, borderSize: number): void {
    const line = viewport.addChild(new PIXI.Graphics());
    line.lineStyle(borderSize, 0xff0000).drawRect(0, 0, viewport.worldWidth, viewport.worldHeight);
}

export function get(): Viewport {
    return viewport;
}
